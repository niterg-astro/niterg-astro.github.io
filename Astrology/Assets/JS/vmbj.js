...
function processDevanagariContent(content) {
    ...
    lines.forEach(line => {
        if (line.endsWith("ध्यायः")) {
            ...
            // Push the current DevTitle if one exists before creating a new one
            if (DevTitle) sirsak.push(DevTitle);
            DevTitle = { devtitle: line.trim() };
            currentsirsak = "";
        } else if (line.trim().match(/॥\s*([१२३४५६७८९०]+)\s*॥$/)) {
...
function displayChapter(index) {
    ...
    const Devsirsak = sirsak[currentChapterIndex];
    let contentHTML = `
    ...
    selectedChapter.slokas.forEach((englishSloka, idx) => {
        let englishtranslation = englishSloka.text
            ...
            .replace(/Note:/g, "<b><h5 class='bg-dark text-light rounded-2 p-2' ><i class='bi bi-paperclip p-2 text-light'></i>Note:</h5></b>")
            .replace(/Commentary:/g, "<b><h5 class='bg-dark text-light rounded-2 p-2' ><i class='bi bi-disc p-2 text-light'></i>Commentary:</h5></b>")
